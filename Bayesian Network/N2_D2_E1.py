...
sand_unit_cost = 25
water_unit_cost = 5
bins = pd.cut(x = data["FCU"].astype(float),bins = 6, labels = ["S1", 
"S2", "S3","S4","S5","S6"],retbins = True)[1]
bins_count = pd.cut(x = data["FCU"].astype(float),bins = 6, labels = ["S1", 
...
evi_fcu['actual'] = np.array([evi_fcu_mean])
evi_fcu['discrete'] = pd.cut(x = evi_fcu["actual"].astype(float),bins = bins, labels = ["S1", 
"S2", "S3","S4","S5","S6"])
    
print(infer.query(['BRAND'], evidence={'Fcu_Discrete':  evi_fcu['discrete'][0],
                                       'AGE': 3,
                                       'GRADE': 4,
                                       'COST': 339.21,
                                       'MM' : 2,
...
print(infer.map_query(['BRAND'], evidence={'Fcu_Discrete':  evi_fcu['discrete'][0],
                                       'AGE': 28,
                                       'GRADE': 2,
                                       'COST': 339.21,
                                       'MM' : 2,
